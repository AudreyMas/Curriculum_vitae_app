          ...
          connexion.query(`SELECT user_id FROM user ORDER BY user_id DESC LIMIT 0, 1`, function (err, results) {
            if (err) {
              console.log('impossible de récupérer id register', err)
                ...
                const accessToken  =  jwt.sign( { id: results[0].user_id } ,'4SchtHE4Zabc!258', {
                expiresIn:  15000
              });
...
postId = (user) => {
  ...
  connexion.query(
    `INSERT INTO curriculum_vitae(user_id) VALUES (${user[0].user_id})`,
    (err, results) => {
      if (err) {
        console.log("error table curriculum_vitae", err);
      } else {
        console.log("table curriculum vitae", results);
      }
    }
  );
} 
